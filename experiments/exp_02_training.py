...
def create_dimension_configs(
    ...
    All models use exactly 1 attention head and 1 layer.
...
def run_dimension_sweep(
    ...
    # All models will train on the same data for fair comparison
    reference_config = configs[0][1]  # Use first config as reference
    ...
    # Use the generate_data function which matches the original training approach
    ...
    print(f"Training {len(configs)} configurations with FIXED training data...")
    ...
    for i, (config_name, config) in enumerate(tqdm(configs, desc="Training models")):
        ...
        try:
            # Train model using fixed training data
            model, losses = train_coinformer_model(config, verbose=False, training_data=(training_datasets, training_priors))
...
def evaluate_dimension_sweep(results, results_dir):
    ...
    for config_name, model in results['models'].items():
        ...
        try:
            ...
            # Store evaluation results (convert tensors to CPU and then to scalars)
